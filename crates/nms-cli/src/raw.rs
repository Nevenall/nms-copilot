...
pub fn run(args: RawArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let json_path = args.path.as_deref().unwrap_or("").trim();
    let segments = parse_path(json_path)?;
    let target = navigate(&root, &segments)?;

    let out = if let Some(needle) = args.find.as_deref() {
        format_find(target, json_path, needle)
    } else if args.keys {
        format_keys(target, args.limit)
    } else {
        serde_json::to_string_pretty(&prune(target, args.depth, args.limit))?
    };
    println!("{out}");
    Ok(())
}
