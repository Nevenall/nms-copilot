...
**Status:** implemented 2026-09-15 on `feature/dashboard` as `dashboard::{run, view, render, input}`, the shared `watch::{drain, sync}` pair with `WatchContext`, the session log, the `dash` command (`dashboard` since 0050, with `dash` kept as an alias), `--prompt`, and the `[dashboard]` table. Drafted the same day, prompted by the fleet arc: the alerts it added only print when the player presses enter, and checking them means switching windows out of the game, which the game handles badly. The player keeps a terminal on a second monitor, so the terminal itself is the notification surface. Departures from the design below: the watcher's channel is checked every 250 ms without a redraw, since a channel cannot wake the crossterm poll; alerts already present when the dashboard opens carry no marker, only ones that arrive afterwards; with `--prompt` or `start = false` an empty line stays at the prompt and only `dash` opens the dashboard; the log's save line names the slot and file written, and a snapshot taken by the watcher is logged too, which settles the last open question.

Revised the same day after first use. The alternate screen is gone: the dashboard is drawn in the terminal's own buffer as an inline ratatui viewport that leaves two rows clear at the bottom, so dropping to the prompt leaves the dashboard on screen with the prompt underneath it and command output scrolls it up like any other output. Returning pushes the screen into the scrollback rather than erasing it and draws a fresh dashboard at the top; a resize draws a fresh screen at the new size. `dashboard::palette` maps the tables' deep-space hexes onto ratatui styles, driven by `display.color`, so the dashboard matches the rest of the output instead of using terminal defaults. A one-line reminder of the way back prints whenever the prompt takes over, because the `dash` command and the empty line were not discoverable. The panels then dropped their box borders for the tables' own shape, a title bar and a header bar over rows on the navy, and the bases and fleet rows took the columns and the exact cell text of the `base` and `fleet` overviews by making `crops_cell`, `extraction_cell`, and `power_cell` public in `nms_query::display`. Since those columns need the width, the two panels stack below 150 columns rather than 100, they take only the rows they need, and the log keeps the rest with its newest line at the bottom. The sections then went back to framed boxes with a themed table inside each, on the user's word that the bar closing a section read as clutter while the frame did not; the REPL's own tables kept their footers. At the same time the sections stopped taking their rows in order, which had left the fleet on a real save showing its column headings and no expedition: `share` now gives every section its minimum first and divides what is left in proportion to what each still wants, and within the fleet the expedition rows outrank the Navigator and home lines. The alerts section then went away entirely: what it said is said in place instead, by drawing a cell in the attention colour when it wants the player, which is where the eye is already looking. That retired the fresh-alert marker and the keypress that cleared it; the bell still rings on a new alert. A player section took the space the alerts had, listing the facts the save holds about the player that the header has no room for: the system and its planet count, the portal address, the distance from the centre, the system warped from, where the freighter is, the three currencies, and what the atlas knows.

**Depends on:** the `base` and `fleet` queries and alerts from project 02 (done), the ratatui screen pattern from the map, and the `nms-watch` watcher with the slot-wide watching from [0047](0047-save-backups.md) (done), so both files of the slot are seen.
...
| `crates/nms-copilot/src/dashboard/palette.rs` | the tables' deep-space colours as ratatui styles |
