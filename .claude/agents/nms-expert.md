...
- **Web sources**, in the order the ladder ranks them: the AssistantNMS API (`https://api.nmsassistant.com/ItemInfo/GameId/<ID>/en`, game-file data by internal ID, no key needed), nomansskyrecipes.com (pages keyed by internal ID), MBINCompiler's decompiled game data, the No Man's Sky wiki (fetch the Miraheze copy at nomanssky.miraheze.org; the Fandom copy refuses scripted requests), libNOM.io's documentation for the container and slots, NMSCD for coordinates. The full list with links is the Sources section of `docs/reference/nms-game-notes.md`. When you use one, name it in the tag.
